...
const PodcastPage: React.FC = () => {
  ...
  const [beyondArticlePodcasts, setBeyondArticlePodcasts] = useState<PodcastItem[]>([]);
    ...
    const fetchSectionData = async () => {
      try {
        ...
        const data = [
          {
            title: 'Interviews',
            podcasts: Array(4).fill(null).map((_, i) => ({
              id: `interview-${i}`,
              title: 'Dark Days for the Petrodollar',
              date: 'October 11, 2023',
              duration: '26:56',
      ...
      {featuredPodcast && (
        <div className="featured-podcast">
          <div className="featured-content">
            <div className="featured-image-container" onClick={() => handlePodcastClick(featuredPodcast.spotifyUrl)}>
              <img src={featuredPodcast.image} alt={featuredPodcast.title} className="featured-image" />
            </div>
            <div className="featured-info">
              ...
              <div className="podcast-duration">
                <span className="duration-icon">▶</span> {featuredPodcast.duration}
              </div>
      ...
      <div className="section">
        ...
        <div className="beyond-article-layout">
          {beyondArticlePodcasts.length > 0 && (
            <div className="beyond-article-main" onClick={() => handlePodcastClick(beyondArticlePodcasts[0].spotifyUrl)}>
              <div className="beyond-article-main-image-container">
                <img src={beyondArticlePodcasts[0].image} alt={beyondArticlePodcasts[0].title} className="beyond-article-main-image" />
              </div>
              <div className="beyond-article-main-info">
                <div className="podcast-date">{beyondArticlePodcasts[0].date}</div>
                <h3 className="podcast-title">{beyondArticlePodcasts[0].title}</h3>
                <div className="podcast-duration">
                  <span className="duration-icon">▶</span> {beyondArticlePodcasts[0].duration}
                </div>
                {beyondArticlePodcasts[0].description && (
                  <p className="podcast-description">{beyondArticlePodcasts[0].description}</p>
                )}
              </div>
            </div>
          )}
          ...
          <div className="beyond-article-sidebar">
            {beyondArticlePodcasts.slice(1, 3).map((podcast) => (
              <div key={podcast.id} className="beyond-article-sidebar-item" onClick={() => handlePodcastClick(podcast.spotifyUrl)}>
                <div className="beyond-article-sidebar-image-container">
                  <img src={podcast.image} alt={podcast.title} className="beyond-article-sidebar-image" />
                </div>
        ...
        <div key={section.title} className="section">
          ...
          <div className="podcast-grid">
            {section.podcasts.map((podcast) => (
              <div key={podcast.id} className="podcast-card" onClick={() => handlePodcastClick(podcast.spotifyUrl)}>
          ...
          <button className="more-episodes-btn" onClick={() => handleMoreEpisodes(section.title)}>
            More episodes
          </button>
        </div>
      ))}
